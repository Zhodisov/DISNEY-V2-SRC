...
class Parse(ParseBase):
    async def fetch(self, dumpscript: List[str]) -> Dict:
        for line, content in enumerate(dumpscript):
            if "getlocal_3" in content:
                if "getlocal_3" in dumpscript[line + 10]:
                    if 'getlocal r4' in dumpscript[line + 13]:
                        if 'getlocal_3' in dumpscript[line + 17]:
                            if 'getlocal_3' in dumpscript[line + 26]:
                                if 'getlocal_1' in dumpscript[line + 27]:
                                    if 'getlocal_1' in dumpscript[line + 31]:
                            
                                                property_line = dumpscript[line + 32]
                                                match = await find_one(GET_PROPERTY_2, property_line)
                                                if match:
                                                    self.results["mouse_speed"] = match.group(1) if match else "Not_found"
                                                    break 
        return self.results
