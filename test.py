                ...
                if "pushscope" in dumpscript[line + 1]:
                    if 'getlex' in dumpscript[line + 2]:
                        if 'getlocal_1' in dumpscript[line + 5]:
                                    if 'returnvoid' in dumpscript[line + 10]:
                                            property_line = dumpscript[line + 3]
                                            match = await find_one(GET_PROPERTY_2, property_line)
                                            if match:
                                                self.results["get_definition"] = match.group(1) if match else "Not_found"
                                                break 
        return self.results
async def main():
    ...
    results2 = await parser2.fetch(dumpscript)

    print(f"Results for dump.txt: {results1}")
    print(f"Results for dump2.txt: {results2}")
